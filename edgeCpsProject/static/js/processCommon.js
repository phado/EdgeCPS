...
function saveAllProject(saveAsProjectName) {
    ...
    if (saveAsProjectName == undefined) {
        var stringWorkflowList = localStorage.getItem(
            projectName + "_workflowXML"
        );
        ...
        workflowProcessList['workflowXML'] = localStorage.getItem(
            projectName + '_workflowXML'
        )
        ...
        var stringWorkflowList = localStorage.getItem(
            saveAsProjectName + "_workflowXML"
        );
        if (stringWorkflowList != null) {
            var modifiedString = stringWorkflowList.replace(/'/g, '"');
            var workflowList = JSON.parse(modifiedString);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let nowPorcess = localStorage.getItem(
        projectName + "_current_processDict"
    ); // 현재 프로세스 확인

    // function createSaveAsButton(buttonName) {   var newButton =
    // document.createElement("button");   newButton.type = "button";
    // newButton.className = "btn btn-primary";
    // newButton.setAttribute("data-bs-toggle", "modal");
    // newButton.setAttribute("data-bs-target", "#exampleModal");
    // newButton.setAttribute(     "data-bs-whatever",     "새로운 프로젝트 이름을 입력해주세요"
    // );   newButton.textContent = buttonName;
    //
    // 스타일 속성 변경   newButton.style.backgroundColor = "#2E5686";
    // newButton.style.color = "#fff";   newButton.style.borderRadius = "5px";
    // newButton.style.padding = "2px 8px";   newButton.style.fontFamily =
    // "'Inter'";   newButton.style.fontStyle = "normal";
    // newButton.style.marginTop = "-2px";   newButton.style.fontSize = "15px";
    // newButton.style.height = "30px";
    //
    //   var buttonContainer = document.getElementById("buttonContainer");
    //
    //   return newButton; } 민수 process 버튼 생성 메뉴 버튼 생성 함수
    function createButton(text, clickFunc, className, style) {
        var button = document.createElement("button");
        button.innerHTML = text;
        button.className = "processButton";
        // button.className = className; 클래스 이름 추가
        if (className) {
            button
                .classList
                .add(className);
    ...
    // buttonContainer.appendChild(createButton("Save
    // All",processSaveClick,"saveButton","font-size:15px; background-color:
    // #2E5686; color: #fff; border-radius: 5px; padding: 2px 8px; font-family:
    // 'Inter'; font-style: normal;"));  process-save 버튼
    // buttonContainer.appendChild(   createButton(     "Save As",
    // openSaveModal,     "saveButton",     "background-color: #3853ff; color: #fff;
    // border-radius: 5px; padding: 2px 8px; font-family: 'Inter'; font-style:
    // normal;"   ) );  process-save 버튼
    // buttonContainer.appendChild(createSaveAsButton("Save As"));  process-save-as
    // 버튼 function openSaveModal() {   var saveModal =
    // document.getElementById("saveModal");   saveModal.style.display = "block"; }
    //
    // buttonContainer.appendChild(createButton("process-load", processLoadClick));
    // process-load 버튼 버튼을 추가할 위치의 요소를 선택 (여기서는 "right_sidebar" 클래스를 가진 div)
    ...
    if (targetElement !== null) {
        ...
        setTimeout(function () {
            ...
            try {
                previousContainer[0].appendChild(
                    createButton("Show requirements", function () {
                        ...
                        window.open(
                            "/previous?ProjectName=" + projectName + "&ProcessName=requirementsProcess",
                            "Popup",
                ...
                if (process_name == "workflowProcess" || process_name == "policyProcess") {
                    previousContainer[0].appendChild(
                        createButton("Show business process", function () {
                            ...
                            // preViewClick(previousXML);
                            window.open(
                                "/previous?ProjectName=" + projectName + "&ProcessName=businessProcess",
                                "Popup",
                                "width=1200, height=700"
                            );
                        })
            ...
            if (nowPorcess == "workflowProcess") {
                workflowSelectList = getWorkflowObjList(localStorage.getItem(
                    projectName + "_" + processXml[2]
                ...
                if (!storedXml || storedXml == "" || storedXml == '<mxGraphModel><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGrap' +
                    'hModel>') {
...
function getLatestXml(strXml) {
    localStorage.setItem(projectName + "_" + localStorage.getItem(
        projectName + "_current_processXml"
    ), strXml); // xml 저장
    if (localStorage.getItem(
        ...
        captureAndDownloadImage(projectName + "_" + localStorage.getItem(
            projectName + "_current_processXml"
        ));
    }
}
...
function getWorkflowData(processGraphxml) {
    localStorage.setItem(localStorage.getItem(
        projectName + "_nowWorkflow"
    ), processGraphxml); // 기존 선택된 워크 플로우 xml
    if (localStorage.getItem(
        projectName + "_current_processDict"
    ) != "policyProcess") {
        //policy일 경우에는 이미지 저장 따로 안함
        captureAndDownloadImage(localStorage.getItem(
            projectName + "_nowWorkflow"
        ));
    }
}
...
function getWorkflowObjList(xml) {
    ...
    // Business Process에서 Edit data에 값이 추가 된 다음 workflow Process로 넘어와서 select box를
    // 생성할 때 오류 발생해서 밑에 새로 짬 var mxCells =
    // xmlDoc.documentElement.getElementsByTagName("object"); if (mxCells.length !=
    // 0) {   for (var i = 0; i < mxCells.length; i++) {     var mxCell =
    // mxCells[i];     if
    // (mxCell.getElementsByTagName("mxCell")[0].attributes[1].nodeValue.includes("rounded=1;")){
    // var id = mxCell.getAttribute("id");        var value =
    // mxCell.getAttribute('name');       var valueString = mxCell.outerHTML;
    // var regex = /\[([^\]]+)\]/g;       var regex = /&quot;&gt;(.+?)&lt;/;
    // var matches = [];        var match;       var match =
    // regex.exec(valueString);       var extractedString = match ? match[1] : null;
    // if (extractedString.includes("[" || "]")) {         extractedString =
    // extractedString.substring(           1,           extractedString.length - 1
    // );       }       matches.push(extractedString);
    //
    // var value = matches;       roundedObjects.push({ id: id, value: value });
    // }   } } else {   for (     var i = 0;     i <
    // xmlDoc.documentElement.getElementsByTagName("mxCell").length;     i++   ) {
    // try {       if (         xmlDoc.documentElement
    // .getElementsByTagName("mxCell")
    // [i].attributes.style.textContent.includes("rounded")       ) {         var
    // inputString =
    // xmlDoc.documentElement.getElementsByTagName("mxCell")[2].attributes
    // .value.nodeValue;         var parser = new DOMParser();         var doc =
    // parser.parseFromString(inputString, "text/html");         var divElement =
    // doc.querySelector("div");         var value = divElement.textContent;
    // if (value.includes("[" || "]")) {           value = value.substring(1,
    // value.length - 1);         }         var id =
    // xmlDoc.documentElement.getElementsByTagName("mxCell")[2].attributes
    // .id.textContent;         roundedObjects.push({ id: id, value: value });
    // }     } catch {}   } }
    //
...
function createWorkflowSelectBox(activityCatList) {
    ...
    if (localStorage.getItem(projectName + "_nowWorkflow") == "" || localStorage.getItem(
        projectName + "_nowWorkflow"
    ) == null) {
        localStorage.setItem(
            projectName + "_nowWorkflow",
            ...
            if (allKeys[j].includes(projectName + '_' + newActivityId)) {
                if (oldActivityName != newActivityName) {
                    ...
                    if (allKeys[j].includes('xml_requirement')) {
                        ...
                        var activityName = activityNameMatch
                            ? activityNameMatch[1]
                            : allKeys[j];
                        const workflowReqText = allKeys[j].substring(allKeys[j].indexOf(
                            activityName + '_'
                        ) + activityName.length + 1);

                        localStorage.setItem(
                            projectName + '_' + newActivityId + '#' + newActivityName + '_' +
                                    workflowReqText,
                            currentLS
                        );
                    } else if (allKeys[j].includes('requirement')) {
                        localStorage.setItem(
                            projectName + '_' + newActivityId + '#' + newActivityName + '_requirement',
                            currentLS
                        );
                    } else if (allKeys[j].includes('nodeSelector')) {
                        localStorage.setItem(
                            projectName + '_' + newActivityId + '#' + newActivityName + '_nodeSelector',
                            currentLS
                        );
                    } else {
                        localStorage.setItem(
                            projectName + '_' + newActivityId + '#' + newActivityName,
                            currentLS
                        );
                    }
    ...
    selectBox.addEventListener("click", function (event) {
        if (event.target.tagName === "OPTION") {
            localStorage.setItem(localStorage.getItem(
                projectName + "_nowWorkflow"
            ), processGraphxml); // 현재 작업 중이던 워크 플로우 xml 저장
...
function insertResult() {
    var data = localStorage.getItem(localStorage.getItem(
        projectName + "_nowWorkflow"
    ) + "_resultLog");
...
function captureAndDownloadImage(workflowName) {
    ...
    // html2canvas(divToCapture).then(function(canvas) {     var link =
    // document.createElement("a")     link.href = canvas.toDataURL("image/jpeg")
    // link.target = "_blank";  파일이 브라우저 창 바깥에서 열리게     link.download =
    // workflowName+'.jpg'  다운로드 할 파일명 설정     link.click()  <a> 요소를 클릭하는 것과 동일한 효과.
    // 브라우저는 해당 링크를 따라가고 href 속성에 지정된 주소로 이동하지 않으면서도 브라우저의 다운로드 동작을 트리거 (링크를 클릭하지
    // 않고도) });
...
function forceApply(graph, cell, value, className) {
    // function forceApply(cellValue){ var FAclassName =
    // cellValue.getAttribute('class');
    var FAcell = cell;
    var FAgraph = graph;
    var FAvalue = value;
    ...
    if (FAclassName.includes('Rounded Rectangle')) {
        ...
        if (typeof(FAvalue) == 'object') {
            ...
            objectElement.setAttribute('inputs.parameters.value', "");
            objectElement.setAttribute('inputs.artifacts.name',"");
            objectElement.setAttribute('inputs.artifacts.value',"");
            objectElement.setAttribute('inputs.artifacts.valueFrom.path',"");
            objectElement.setAttribute('outputs.artifact.name', "");
            objectElement.setAttribute('outputs.artifact.path', "");
...
function parseString(inputString, processName, cellId) {
    ...
    if (processName === 'workflowProcess') {
        ...
        // if(match[1].includes('['||']')){   result
        // =match[1].substring(1,match[1].length -1);
        result = projectName + '_' + cellId + '#' + result;
        // }else{   result = projectName+'_'+cellId+'#'+result; } return '';
        ...
        result = localStorage.getItem(projectName + '_nowWorkflow') + '_' + cellId +
                '#' + result;
    }
    return result;
}