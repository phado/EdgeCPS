...
EditorUi = function (editor, container, lightbox) {
  ...
  if (!graph.standalone) {
    ...
    function openSmallPopup() {
      // 팝업 창의 속성 설정
      var popupFeatures =
        "width=300,height=200,toolbar=no,location=no,scrollbars=no";
    ...
    var textEditing = mxUtils.bind(
      this,
      function (
        evt //민수 여기에서 evt.path 안에 내가 클릭한 도형이 들어간다.
      ) {
        if (evt == null) {
          evt = window.event;
        }
    ...
    this.editor.resetGraph(); //민수 저부 세팅 하고 한번 새로고침 우측사이드바는 아직 입력 안됨
  }

  this.init(); //민수 여기에서 우측 사이바 입력 됨
...
EditorUi.prototype.showShapePicker = function (
  ...
  source,
  callback,
  direction // 민수 여기도 우측 사이드바 생성 되는 곳과 관련 ㅇㅇ
) {
...
EditorUi.prototype.refresh = function (sizeDidChange) {
  ...
  if (this.menubar != null) {
    this.menubarContainer.style.top = this.menubarHeight + 100 + 40 + "px"; //순우 상단 메뉴바 재배치
    this.menubarContainer.style.height = this.menubarHeight + "px";
    tmp += this.menubarHeight;
  }

  if (this.toolbar != null) {
    this.toolbarContainer.style.top = this.menubarHeight + +100 + 70 + "px"; //내림
...
EditorUi.prototype.createUi = function () //민수 요청된 ui생성하는 곳
{
  ...
  // 순우 우측 사이드바 판넬 생성 부분
  if (this.format != null) {
    // this.formatContainer.className = 'rightProperty'
  ...
  // Creates toolbar 민수 상단 툴바 생성되는 부분
...
EditorUi.prototype.createStatusContainer = function () {
  ...
  container.className = "geItem geStatus"; // 메뉴바 부분
...
EditorUi.prototype.destroy = function () {
  ...
  if (process_name == "overviewProcess" || process_name == "runProcess") {
    var c = [
      /*순우 상단 메뉴바 삭제*/ this.menubarContainer,
