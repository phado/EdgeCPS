 ...
 */
Sidebar.prototype.init = function()// 순우 각 프로세스 별 불러올 좌측 사이드바 선택 하는 곳
{
...
if (urlParams['sidebar-entries'] != 'large')
{
	...
	if(process_name=='requirementsProcess'){
		Sidebar.prototype.thumbWidth = 182;
...
// 순우 businessprocess 좌측 사이드바 만들기
Sidebar.prototype.addGeneralPaletteBusinessProcess = function(expand)
{
	var lineTags = 'line lines connector connectors connection connections arrow arrows ';
	this.setCurrentSearchEntryLibrary('general', 'general');
	var dt = 'uml static class ';
...
Sidebar.prototype.addGeneralPaletteWorkflow = function(expand)
{
	...
	this.addPaletteFunctions('general', mxResources.get('workflowSideBar'), (expand != null) ? expand : true, fns);
	this.setCurrentSearchEntryLibrary();
};
...
Sidebar.prototype.addUmlPalettePolicy = function(expand)
{
	// Avoids having to bind all functions to "this"
	var sb = this;
...
Sidebar.prototype.createThumb = function(cells, width, height, parent, title, showLabel, showTitle, realWidth, realHeight)
{
	...
	// 순우 좌측 사이드바 아이콘 컨테이너 크기 조절 하는 곳
...
Sidebar.prototype.createItem = function(cells, title, showLabel, showTitle, width, height, allowCellsInserted)
{
	...
	elt.style.width = (this.thumbWidth + border) + 20+'px';
	elt.style.height = (this.thumbHeight + border) +20+'px';
	if(process_name =='requirementsProcess'){
		elt.style.padding = this.thumbPadding + '0'+'px';
		elt.style.marginLeft = '-5px';
	}else{
		elt.style.padding = this.thumbPadding + '0'+'px';
		elt.style.marginLeft = '15px';
	}
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createDropHandler = function(cells, allowSplit, allowCellsInserted, bounds) // 민수 요소 최종 생성하는 곳
{
		...
		if (elt == null && graph.isEnabled())
		{
			...
			if (cells.length > 0)
			{
				...
				if (!graph.isCellLocked(target || graph.getDefaultParent()))
				{
					...
					{
						graph.model.endUpdate(); // 민수 요소가 생성 되는 초종 함수.
					}
			...
			mxEvent.consume(evt); // 민수 객체 클릭시 여기서 리턴함
		}
	});
};
 ...
 * Creates a drag source for the given element.
 */
Sidebar.prototype.createDragSource = function(elt, dropHandler, preview, cells, bounds) // 주어진 요소를 생성하는 곳 민수
{
 ...
 */
Sidebar.prototype.itemClicked = function(cells, ds, evt, elt) // 클릭할때 이벤트 발생 시킴 민수(요소를 클릭하든 뭘하든)
{
	var graph = this.editorUi.editor.graph;
